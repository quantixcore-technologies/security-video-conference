...
- ✅ **Android и Tauri** — закрыто в S39 (раздел ниже).
...
## 💬 2026-09-13 — помощник в Android и Tauri (S39) + Android v0.5.2 обязательный ✅
Запрос заказчика: «yordamchini android va tauri ga ham qo'sh». **Эпик E8 закрыт (2/2).**
- ✅ **Один API на три клиента.** Android и Tauri ходят в тот же `/api/assistant/{suggestions,ask,:id}`,
  что и веб-виджет: поиск, роли и тексты живут только на сервере, клиент лишь рисует ответ
  (`ok` + связанные темы · `unsure` — список · `restricted` · `no_match` — подсказки). Поправить
  базу знаний = задеплоить бэкенд; выпускать приложения ради этого не нужно.
- ✅ **Android** (`Assistant.kt`, `SvcApi.kt`): кнопка в правом нижнем углу главного экрана → нижний
  лист с чатом; нажатие на подсказку открывает тему по `id`, без повторного поиска. Язык — по языку
  устройства (uz/ru/en).
- ✅ **Tauri** (`main.ts`): кнопка 💬 в панели звонка → боковая панель. Звонок — единственный экран
  клиента после входа, поэтому помощник живёт там.
- ✅ **Роли по-человечески:** в ответе `restricted` появилось `allowed_role_labels` — названия ролей на
  языке пользователя вместо внутренних кодов, во всех трёх клиентах. Бэкенд задеплоен 09:18 UTC,
  проверено на проде (uz/ru/en). **317 тестов, 0 failures**; CI зелёный по всем трём стекам.
- 🐛 **Эмулятор поймал то, что пропустили lint и CI:** `ModalBottomSheet` по умолчанию открывается
  наполовину, и поле ввода оказывалось за нижним краем экрана — пользователь не видел, куда писать.
  Исправлено (`skipPartiallyExpanded` + `imePadding`) и перепроверено с открытой клавиатурой: строка
  ввода поднимается над ней. Ловушка: эмулятор по умолчанию не рисует экранную клавиатуру
  (`show_ime_with_hard_keyboard`), и без этой настройки скриншот «с клавиатурой» ничего не доказывает.
- ✅ **Android v0.5.2 — ОБЯЗАТЕЛЬНОЕ** (`minVersionCode: 10`), релиз 09:34 UTC: подпись `3d1cdad9…`
  совпала, versionCode внутри APK сверен с `version.json` (новая проверка в `release_apk.sh`), блок `ios`
  на месте; прежние файлы — `~/apk-backup/*.20260913-093450`.
- ✅ **Первый настоящий прогон D-020:** вручную ничего не рассылалось — в 09:40:01 UTC, на ближайшем
  тике cron, сервер сам записал 0.5.2 в `app_release_announcements` и отправил «Ilovani yangilang»
  **4 из 4** активным пользователям, без дублей.
- ⚠️ **Честная граница:** Tauri-панель проверена только сборкой (`tsc`, `vite build`, CI) — у
  desktop-клиента нет канала раздачи, вживую её никто не видел.

## 🔐 2026-09-14 — hardening к продакшену (шаг 1 «путь к product»)
- 🐛→✅ **КРИТИЧНО: подписной ключ утекал в публичный репозиторий.** Прод работает в `MIX_ENV=dev`,
  а `config/dev.exs` держит хардкод `secret_key_base`, который лежит в открытом GitHub. Этим ключом
  можно было подписать bearer-токен/сессию за **любого** пользователя (проверено: токен, подписанный
  этим ключом для `user_id:1`, давал `/api/me` → 200 как super_admin). Исправление: `runtime.exs`
  теперь читает `SECRET_KEY_BASE` из env во **всех** окружениях (не только `:prod`); на сервере в
  `~/svc-real.env` записан свежий ключ, которого нет в репозитории. После ротации: токен со старым
  (публичным) ключом → **401**, легит?ный вход (`/api/login` → `/api/me`) → **200**. Все bearer/сессии
  инвалидированы — клиенты перелогинятся (Android `me()`→clear→re-login, graceful).
- ✅ **CLOAK_KEY тоже вынесен в env** (был публичный dev-fallback, шифрует `totp_secret` at-rest).
  Единственный неиспользуемый `totp_secret` (totp_enabled=false) обнулён до ротации ключа.
...
## 🧪 2026-09-16 — путь к финальному продукту (по порядку)
- ✅ **Восстановление из бэкапа ПРОВЕРЕНО.** Borg-репо `/mnt/backup/borg` (458G, 2%), в архиве
  `pg_dumpall` (`var/backups/svc-db/all-databases.sql.gz`). Тест (без касания прода): извлёк дамп из
  последнего архива → gzip OK → восстановил секцию БД `svc` во **временную** `svc_restore_test` →
  счётчики **точно совпали с живыми** (users=4, meetings=2, orgs=1, notifications=17, роли верны) →
  временную БД удалил. Данные восстанавливаемы. (Процедура — в runbook, шаг 3.)
- ✅ **Security-review проведён.** Главная находка (HIGH, исправлено+задеплоено+проверено вживую):
  **IDOR в join** — `/api/meetings/:id/join` пускал в звонок по org_id без проверки видимости, т.е.
  участник той же орг мог войти в чужую ЗАКРЫТУЮ встречу по перебору id и получить LiveKit-токен.
  Фикс (D-021): `can_view_meeting?` (организатор/приглашённый), не-видимую → 404. Прод-проверка:
  организатор 200, чужой той же орг 404, после приглашения 200. Остальное чисто: API под auth, нет
...
## 📄 2026-09-19 — S40: обмен документами (закрыт разрыв «дека обещает / кода нет»)
- В деке и README «secure document exchange» значился как возможность, а кода не было
  **вообще** (ни таблицы, ни API, ни экрана). Теперь работает.
- **Что есть:** загрузка файла (до 25 МБ) с шифрованием at-rest (`Svc.Vault`, AES-GCM),
  явный список получателей, срок действия, отзыв, версии с наследованием получателей,
  отметка «кто и когда открыл», аудит всех операций (`document_upload/download/revoke`).
- **Доступ** — как у встреч (D-016/D-021): чужой из той же организации не видит; невидимое
  отдаётся как `not_found`. `super_admin` может только **отозвать**, но не прочитать (D-022).
- **Где:** контекст `Svc.Documents` + `Svc.Documents.Storage`, экран `/admin/documents`,
  скачивание — контроллер (всегда `application/octet-stream` + `nosniff`).
- **Найдено и исправлено в процессе:** Sobelow поймал `XSS.ContentType` — отдача MIME,
  пришедшего от клиента, дала бы хранимую XSS. Починено кодом, не подавлением.
- 26 новых тестов (контекст + web), `mix precommit` зелёный (346 тестов, credo 0, Sobelow 0 high/med),
  переводы uz/en заполнены (0 непереведённых).
- **Не входит:** подпись документов (E-IMZO) — остаётся в roadmap; экран документов в мобильных
  клиентах — следующий слайс.

## 📬 2026-09-19 — S41: «от кого и что с этим делать» + документы в Android
- К документу добавлена **резолюция** (к сведению / на рассмотрение / на подпись / на исполнение),
  **срок исполнения** и комментарий отправителя. Получатель ставит отметку «ознакомился» —
  она отличается от «скачал»: отправителю важно видеть, что работа принята.
- **Уведомление** получателю при загрузке (kind `document`): в тексте — резолюция, от кого и срок,
  поэтому мобильный клиент показывает суть, не открывая документ.
- **REST для клиентов:** `GET /api/documents`, `GET /api/documents/:id`, `/download`, `POST /:id/ack`.
  Права целиком в контексте — API не ходит мимо тех же границ, что и веб.
- **Android:** новая вкладка «Hujjatlar» — кто прислал, что сделать (подписи на узбекском),
  срок, комментарий, открытие файла системным приложением (FileProvider, кэш), кнопка «Tanishdim».
  Проверено на эмуляторе: отправитель видит «Siz yubordingiz», получатель — «Kimdan: Aziz Karimov».
- Прод: миграция применена, релиз перезапущен, API проверен живьём. **APK v0.5.4 (12)** выпущен,
  уведомление пользователям уходит автоматически (D-020).
- 353 теста, 0 падений; precommit зелёный.

## 🔑 2026-09-20 — S42: вход при нескольких организациях + демо-доступ для жюри
- **Найденный дефект:** данные мультитенантные (D-005), а логин — нет: и веб, и API звали
  `Orgs.default_organization()`, то есть организацию с наименьшим id. Вторая организация на том же
  сервере **не могла войти вообще**. Для платформы «одна установка — несколько ведомств» это дефект.
- **Исправлено:** `Accounts.authenticate/2` ищет пользователя по логину среди всех организаций и
  пускает того, чей пароль сошёлся (имя уникально в пределах org, тёзки разводятся паролем).
  Ответ при неверном пароле и при несуществующем логине одинаков — организация не раскрывается.
  6 тестов: вторая орг входит, тёзки попадают каждый к себе, disabled/lockout/регистр.
- **Демо-доступ для жюри (третий тур):** отдельная **изолированная организация** «Demo tashkiloti»
  с тремя аккаунтами (`demo` — руководитель, `demo.xodim` — сотрудник, `demo.admin` — админ,
  пароль `DemoSvc2026!`) и наполнением: встреча, документ с резолюцией «на подпись», поручение.
  Проверено: демо-аккаунты видят только свою организацию, реальные пользователи не затронуты.
- Логин и сценарий «что посмотреть за 3 минуты» опубликованы на `svc.co1nlist.uz/demo/`
  (страница, на которую ведёт ссылка в заявке).

## 🛡 2026-09-20 — публичная страница «Безопасность и соответствие» (третий тур)
- `svc.co1nlist.uz/security/` — не обещания, а **что реально сделано**, с указанием **как проверить**
  каждый пункт: где лежат данные (сервер в Узбекистане, Ургенч, Uzbektelekom AS8193), чем шифруется
  (DTLS-SRTP, AES-256-GCM для файлов, Argon2id), разграничение доступа, неизменяемый аудит,
  матрица анти-захвата по платформам, бэкап с **проверенным** восстановлением, автолечение, качество кода.
- **Честный раздел «Открытые недостатки»** — эксперт третьего тура всё равно это спросит:
  медиа пока идёт через LiveKit Cloud (решение — self-host LiveKit, архитектура готова);
  E2EE/SFrame в roadmap; E-IMZO нет; госсертификации и внешнего пентеста нет; HA нет; iOS без Apple-аккаунта.
- **Найдено при подготовке:** `Accounts.totp_required?/1` («2FA обязательна для руководства») существует и
  покрыт тестом, но **в логине не используется** — админ без 2FA входит по паролю. Не стали выдавать
  желаемое за действительное: на странице это указано как открытый недостаток, включение — следующий слайс.
- Ссылка со страницы демо; копия страницы — в `docs/security/public/`.

## 🕒 2026-09-21 — S43: встречу открывает человек и он же её закрывает + история

**Требование заказчика.** «Майлис бир киши томонидан очилиши ва худди шу ишчи ходим
томонидан тугалланиши лозим… мажлис қачон соат нечидан нечигача бўлиб ўтгани тарихини ҳам
ёзиб қўйиши керак ва мажлис нима учун бўлганлигини ҳам… ундан ташқари аккаунтга кираётган
пайтда паролни ёзганда ёнида кўз бўлсин».

- **БД/схема** (`20260921120000_add_meeting_session_history`): `started_at`, `ended_at`,
  `started_by_id`, `ended_by_id`, `purpose`, `summary`. Плановые `scheduled_*` остались как
  план — факт хранится отдельно.
- **Контекст** `Svc.Meetings`: `open_meeting/2`, `close_meeting/3`, `can_control?/2`,
  `can_close?/2` (первый — открывший; организатор/super_admin запасные, иначе встреча
  зависнет «в эфире» при обрыве связи), `history/2` (только завершённые, фильтр видимости
  D-016, preload организатор/открывший/закрывший). Оба действия пишутся в аудит
  (`meeting_open`/`meeting_close` + длительность). Решение — **ADR D-023**.
- **REST**: `POST /api/meetings/:id/open`, `POST /api/meetings/:id/close` (`summary`),
  `GET /api/meetings/history`; в списке встреч появились `purpose/started_at/can_open/can_close`.
  Невидимая встреча — 404 (не раскрываем существование), нет прав — 403, повтор — 409.
- **Веб**: кнопки «Начать встречу» / «Завершить встречу» на карточке встречи, форма итога,
  блок «История встречи» (факт. время, длительность, кто открыл/закрыл, повод, итог),
  поле «Повод для встречи» в формах создания и редактирования.
- **Android** (`MainActivity`/`SvcApi`): переключатель «Joriy / Tarix», кнопки
  «Majlisni boshlash» / «Majlisni yakunlash» (видны только тому, кто вправе), диалог итога,
  карточка истории, поле «Majlis nima uchun?» при создании. Время показывается в часовом
  поясе устройства (факт хранится в UTC).
- **iOS** (`MeetingsView`/`SvcApi`): тот же паритет — сегмент «Joriy/Tarix», кнопки
  открытия/завершения, sheet итога, карточка истории, поле повода.
- **Вход в веб**: кнопка-глаз у поля пароля (`id="login-password"`, `data-password-toggle`).
  В Android и iOS такая кнопка уже была — теперь все три клиента одинаковы.
- **Тесты**: 24 новых (13 контекст, 7 API, 3 LiveView, 1 страница входа). `mix precommit`
  зелёный: **383 теста, 0 failures**, компиляция без предупреждений, Sobelow без Medium+.

## 🔔 2026-09-21 — S44: время начала встречи + кнопка «позвать опаздывающих»

**Требование заказчика.** «Мажлис қачон бошланиши ёзиш лозим ва кечга қолиши мумкин
бўлган ишчи ходимлар бор — уларга ҳам алоҳида кнопка орқали бильдиришнома, яъни
мажлисга келинг дейдиган бильдиришнома юбориш керак».

- **Время начала.** В вебе поле было, в мобильных — нет: встречи создавались без
  расписания, из-за чего не работали ни напоминания (Oban T-24ч/T-1ч), ни расчёт
  опозданий (`late_threshold_seconds`). Теперь выбор времени есть в Android
  (системные DatePicker/TimePicker) и iOS (`DatePicker`). Формат — **локальное
  «стенное» время без зоны** (`2026-10-01T09:30`), ровно как шлёт веб-форма
  `datetime-local`; иначе встреча съезжала бы на 5 часов.
- **«Позвать на встречу».** `Attendance.pending_invitees/1` — приглашённые без записи
  о входе в звонок; `Meetings.call_participants/3` шлёт им уведомление (`:reminder`,
  текст по-узбекски — его читают на телефоне) и пишет аудит `meeting_nudge` с числом
  позванных. Того, кто уже в звонке, не трогаем. Право — у организатора, открывшего
  встречу и super_admin. **Кулдаун 60 секунд** (проверяется по аудиту) — чтобы
  повторные нажатия не превращались в спам-рассылку.
- **UI.** Веб: «Позвать всех (N)» в шапке ростера + кнопка напротив каждого, кто ещё
  не зашёл, и отметка «в звонке» у остальных. Android/iOS: кнопка
  «Majlisga chaqirish (N)» на карточке встречи (видна только тому, кто вправе);
  число берётся из нового поля `pending_count` в `GET /api/meetings`.
- **Тесты**: 18 новых (10 контекст, 5 API, 3 LiveView). `mix precommit` зелёный:
  **401 тест, 0 failures**.

## ✂️ 2026-09-22 — форма встречи упрощена по просьбе заказчика (D-024)

- Убрано поле **«Конец»**: «тугаш вақтини мажлис бошланмасидан олдин билиб бўлмайди ку».
  Фактическая длительность и так считается по S43 (`started_at`/`ended_at`).
- Убран блок **«Повтор / Повторений»** и вместе с ним `Meetings.create_recurring/4`,
  `save_recurring` и 5 тестов серий. Колонка `recurrence_group` осталась для старых встреч.
- В форме теперь: название · повод · начало · запись · участники.

## 🚪 2026-09-22 — S45: вход в звонок «один раз вышел — предупреждение, второй — закрыто»

**Требование заказчика** (три пункта одним сообщением):
1. уведомление о встрече должно ОТКРЫВАТЬ встречу по нажатию;
2. завершённых встреч в списке быть не должно — только предстоящие и идущие;
3. участник не может свободно выходить и возвращаться: первый выход — предупреждение
   и ещё одна попытка, второй — вход закрыт и человек под подозрением.

- **Ограничение входа** (ADR **D-025**): считаем подтверждённые выходы
  (`leave_count` из вебхука `participant_left`), а не выданные токены.
  `Attendance.rejoin_state/2` → `:ok` | `{:warn, :last_attempt}` | `{:blocked, record}`;
  `Meetings.join_guard/2` пропускает ведущего (организатор / открывший / super_admin),
  иначе при обрыве связи встречу стало бы некому завершить. Второй выход: отметка
  `flagged` в посещаемости, событие `rejoin_blocked` в журнале `/admin/security`,
  аудит `meeting_rejoin_blocked` и уведомление организатору. API отдаёт 403 с полем
  `message`, веб-страница звонка — флеш и редирект.
- **Список встреч.** `Meetings.list_active_meetings/1` (планируемые + идущие) в API;
  в вебе фильтр по умолчанию исключает завершённые, но по явному выбору статуса их
  по-прежнему видно. Завершённые живут во вкладке «Тарих» (S43).
- **Уведомление → встреча.** В Android и iOS карточка уведомления с `meeting_id`
  стала кнопкой: отмечает прочитанным и сразу подключает к звонку (стрелка на карточке).
  В вебе ссылка «К встрече» была и раньше.
- **Тесты**: 16 новых. `mix precommit` зелёный: **412 тестов, 0 failures**.
...
- **Помощник в Tauri на Windows** — панель S39 проверена только сборкой; при первом Windows-прогоне
  (вместе с детектором и `setContentProtected`) посмотреть её вживую.
...
## 📚 Ключевые доки
ADR: `docs/ARCHITECTURE_DECISIONS.md` (D-001…D-023) · разведка: `docs/research/` (4) · спеки: `docs/superpowers/specs/E0-E7` · итоги: `docs/sessions/2026-06-05.md` · `CLAUDE.md`(навигация).
