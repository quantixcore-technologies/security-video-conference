...
- 🔧 **Инфраструктурные уроки** (подробно — `coinlist:~/DAVOM.md`): SSH к проду работает только
  в manual-режиме Claude Code (auto-классификатор блокирует); Cloudflare-туннель рвёт «тихие»
  долгие команды — обязательно `ServerAliveInterval=15` (именно так оборвался первый прогон:
  между compile и restart); `deploy_update.sh` **не запускать** — staging-лендинг старее
