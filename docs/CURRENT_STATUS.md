...
## ✅ Сделано после редизайна (58 коммитов, 161 тест 0 failures)
...
- **E4 поручения/задачи (A):** `tasks` (creator→assignee, meeting_id, priority, status todo/in_progress/review/done, due_at) · `Svc.Tasks` (create/set_status/board/open_count) · ADR **D-015** («CRM» для гос = поручения+задачи, НЕ sales-CRM). _(Kanban-UI = E4-B следующий; обращения граждан 🔒 заказчик)._

## ⏭️ СЛЕДУЮЩИЙ КВЕСТ: E4-B Kanban-доска
- **E4-B:** Kanban-доска UI (колонки по статусам Новые/В работе/Проверка/Выполнено, карточки, drag-drop, создание поручения) — видимый результат для руководителя.
- Далее E4: **C** (связь со встречей + E3-уведомление исполнителю) · **D** (отчётность).
...
DB_PORT=5434 mix test                                        # 161 тест 0 failures
