...
**Решение (Otabek, интервью 2026-06-05):** «CRM» в SVC = **система поручений руководства + задач сотрудникам** (единый task/assignment-модуль с Kanban-доской). Поручения часто рождаются из совещаний → связь с встречами (E1/E2). **Обращения граждан** — отдельный возможный модуль, отложен (требует уточнения у заказчика). **Обоснование:** для гос-ведомства «CRM» ≠ продажи; реальная потребность — контроль исполнения поручений и задач. **Последствия:** E4 строим как Tasks (поручение/задача: автор→исполнитель, дедлайн, приоритет, статус) + Kanban-UI + интеграция с E3-уведомлениями; обращения граждан — потом, по требованию.
