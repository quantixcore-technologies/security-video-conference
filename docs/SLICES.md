...
## 📊 Прогресс: **44 из 45 нумерованных слайсов готовы** (S1–S45; в работе только S34 — iOS 🔵, ждёт Apple Developer)
...
| S34 | iOS-клиент (SwiftUI): landing/login/2FA, встречи + создание, уведомления, отдел, профиль, Keychain, GPS, **isCaptured/screenshot detect → /api/capture-events**, **видео/аудио звонок (LiveKit `CallView`)**, **встроенный помощник S39 (`AssistantView` + `/api/assistant/*`, паритет с Android)** | 🔵 код-паритет с Android достигнут, **CI (macOS runner) зелёный** (2026-09-16), контракт помощника сверен с прод; остаётся ТОЛЬКО установка на живой iPhone — нужен Apple Developer ($99/год) |
...
| S39 | **Помощник в Android и Tauri** — тот же REST `/api/assistant/*` (поиск, роли и тексты — только на сервере) · **Android** (`Assistant.kt`): кнопка в правом нижнем углу главного экрана → нижний лист-чат: подсказки-чипы, ответ + связанные темы, список при `unsure`, роли при `restricted`, подсказки при `no_match`; язык — по языку устройства · **Tauri** (`main.ts`): кнопка 💬 в панели звонка (единственный экран клиента после входа) → боковая панель · `restricted` отдаёт `allowed_role_labels` — названия ролей на языке пользователя вместо кодов, во всех трёх клиентах · **Android v0.5.2 — обязательное обновление** (`minVersionCode: 10`) | ✅ 2026-09-13 · +1 контрактный тест API (всего 317, 0 failures) · Android прогнан на эмуляторе от входа до ответа — поймал баг, пропущенный lint и CI (поле ввода за нижним краем экрана) · Tauri — только tsc + vite build в CI, визуально не проверен |
| S40 | **Обмен документами** — загрузка файла (шифрование at-rest через Cloak, блоб вне веб-корня), явный список получателей, срок действия, отзыв, версии, отметка «кто открыл», аудит всех операций · LiveView `/admin/documents` + аутентифицированное скачивание контроллером (всегда `application/octet-stream` + `nosniff` — иначе загруженный `text/html`/SVG дал бы хранимую XSS) | ✅ 2026-09-19 · 26 тестов (контекст + web), precommit зелёный |
| S41 | **Документы «от кого / что делать»** — резолюция (к сведению · на рассмотрение · на подпись · на исполнение), срок исполнения, комментарий отправителя, отметка «ознакомился» (отдельно от «скачал») · уведомление получателю (kind `document`) · REST `/api/documents` (+download, +ack) · **вкладка «Hujjatlar» в Android** с узбекскими подписями резолюций и открытием файла системным приложением | ✅ 2026-09-19 · проверено на эмуляторе и на проде; APK v0.5.4 |
| S42 | **Вход при нескольких организациях** — `Accounts.authenticate/2` подбирает организацию по самому пользователю (имя уникально в пределах org, тёзок разводим паролем; ответ одинаков при неверном пароле и несуществующем логине). Раньше вход шёл только в `default_organization` — вторая организация войти не могла в принципе. Плюс изолированная **демо-организация** для жюри (3 аккаунта, свои встреча/документ/поручение) | ✅ 2026-09-20 · 6 тестов, проверено на проде |
| S43 | **Встречу открывает человек — и закрывает её тот же человек** — `open_meeting/2` (кто открыл + фактическое время), `close_meeting/3` (кто закрыл + итог), `can_close?/2` (первый — открывший; организатор/super_admin запасные), история завершённых «когда · сколько · зачем · итог» с фильтром видимости D-016 · REST `POST /api/meetings/:id/open|close` + `GET /api/meetings/history` · кнопки «Начать/Завершить встречу», поле «повод» и карточка истории в вебе, Android и iOS · **кнопка-глаз для пароля на веб-входе** (в Android/iOS уже была) | ✅ 2026-09-21 · 24 теста (13 контекст + 7 API + 3 LiveView + 1 страница входа), precommit зелёный (383 теста) |
| S44 | **Время начала + «позвать на встречу»** (2026-09-22: из формы убраны «Конец» и «Повтор» — D-024) — `Attendance.pending_invitees/1` (кто ещё не заходил в звонок), `Meetings.call_participants/3` (уведомление опаздывающим; зовёт организатор/открывший/super_admin; минутный кулдаун от спама, аудит `meeting_nudge`) · REST `POST /api/meetings/:id/nudge` + `pending_count` в списке · веб: «Позвать всех (N)» и кнопка напротив каждого + отметка «в звонке» · Android и iOS: выбор времени начала при создании (локальное «стенное» время, как в веб-форме) и кнопка «Majlisga chaqirish (N)» | ✅ 2026-09-21 · 18 тестов, precommit зелёный (401 тест) |
| S45 | **Ограничение повторного входа + чистый список + переход из уведомления** — `attendance_records.join_count/leave_count/flagged`, `Attendance.rejoin_state/2`, `Meetings.join_guard/2` (1-й выход — предупреждение, 2-й — вход закрыт + событие `rejoin_blocked` в журнале безопасности + уведомление организатору; ведущий не блокируется — D-025) · в списке встреч только запланированные и идущие (завершённые — в истории) · нажатие на уведомление в Android и iOS открывает встречу и сразу подключает к звонку | ✅ 2026-09-22 · 16 тестов (11 контекст + 4 API + 1 LiveView), precommit зелёный (412 тестов) |
