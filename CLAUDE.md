...
## Что это
B2G (Узбекистан) secure видеоконференц-платформа. Команда: Furqat / Shuxrat.
...
9. **Gettext мержить только `mix gettext.merge priv/gettext --no-fuzzy`.** Fuzzy-сопоставление
   подставляет перевод СОСЕДНЕЙ строки: так «Пароль» на странице входа месяцами показывался
   как «Профиль», а кнопка «Войти» — как «Выйти» (исправлено 2026-09-21). В ru-каталоге
   (язык-источник) `msgstr` обязан совпадать с `msgid` либо быть пустым.
